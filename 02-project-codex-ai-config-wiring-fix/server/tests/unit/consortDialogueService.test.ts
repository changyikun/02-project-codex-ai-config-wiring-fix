...
describe('ConsortDialogueService', () => {
  it('把 AI 生成的无选项问句兜底转成可回应分支', async () => {
    const service = buildService(async () => ({
      ...
      speakerIdentity: '贵妃',
      speakerName: '姚铃儿',
      text: '姚铃儿将茶盏一转，忽而抬眼问：“小主今日这样问我，是想听真话，还是想看我如何回避？”',
      sceneHint: '她把话递了回来。',
      options: [],
      memoryCandidates: [],
      affectHints: [],
  ...
  it('把没有问号但明显索要表态的台词也兜底转成分支', async () => {
    const service = buildService(async () => ({
      ...
      speakerIdentity: '贵妃',
      speakerName: '姚铃儿',
      text: '姚铃儿将杯沿轻轻一按，只抬眼望着你道：“小主若真有心来试我，便给妾一句准话。”',
      sceneHint: '她把话压到了你面前。',
      options: [],
      memoryCandidates: [],
      affectHints: [],
  ...
  it('会把送礼与 AI 候选整理成结构化 relationCandidates', async () => {
    ...
    const service = buildService(async () => ({
      ...
      speakerIdentity: '贵妃',
      speakerName: '姚铃儿',
      text: '姚铃儿接过香囊，指尖在穗线上轻轻一拂，像是把那一点惊讶也按了下去：“小主这份心，妾会记着。”',
      sceneHint: '她把情绪压得很轻。',
      options: [],
      memoryCandidates: [
        {
  ...
  it('允许 AI 主动终结话题，并统一转成收起动作', async () => {
    const service = buildService(async () => ({
      ...
      speakerIdentity: '贵妃',
      speakerName: '姚铃儿',
      text: '姚铃儿把茶盏轻轻搁回案上，语气仍旧端得极稳：“小主今日问到这里，妾该回的也都回了。再往下说，倒像是彼此都失了分寸，今日便先到这里吧。”',
      sceneHint: '她主动把话题收住。',
      options: [],
      memoryCandidates: [],
      affectHints: [],
  ...
  it('同一 session 接近二十轮后会自动收束话题', async () => {
    ...
    const service = buildService(async () => {
      ...
      return {
        ...
        speakerIdentity: '贵妃',
        speakerName: '姚铃儿',
        text: '姚铃儿把话音压得很稳：“小主这句话，妾听明白了，只是眼下还不宜说得更深。”',
        sceneHint: '话头仍在延续。',
        options: [],
        memoryCandidates: [],
        affectHints: [],
  ...
  it('相同 dedupeKey 的长期关系不会重复写入', async () => {
    ...
    const service = buildService(async () => ({
      ...
      speakerIdentity: '贵妃',
      speakerName: '姚铃儿',
      text: '姚铃儿把香囊收进袖中，语气仍旧端着：“既是小主的心意，妾自然记得。”',
      sceneHint: '她把这份心意记下了。',
      options: [],
      memoryCandidates: [
        {
