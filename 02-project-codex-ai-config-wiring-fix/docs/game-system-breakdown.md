...
- 乐理值：读取既有玩家属性字段 `state.stats.talent`，该字段在界面和规则中显示为“乐理”。若数值大于 `10`，按 `/10` 折算到 `0..10`。
...
- 学谱成长：每次学谱额外增加 `talent +0.2`，按副属性展示倍率显示为“乐理 +2”。
...
- 公共外出地点从 `MAP_HOTSPOTS` 派生，排除“后宫”聚合入口和玩家寝殿占位；御花园、正阳门、重华宫、御书房、宫门、建章宫、冷宫、养心殿等空地图点也属于可能出现 NPC 动向的地点。
- 妙音堂、御膳房、太医院、宝华殿、华清池等功能小场景优先读取行动表中的安排；没有安排时，不允许再从全体 live 妃嫔里无条件随机抓人。
- 地图热点卡可以显示本旬安排在该地点的妃嫔摘要；真正进入公共地点或空地图点后，NPC 应显示为可点击交互入口，玩家主动点击后再用地点入场叙述和行动摘要进入妃嫔日常对话。
