...
- 已处理：按钮本体改为透明，竖牌素材只绘制在直接子元素上；竖向热点宽度按 `shukuang1.png` 原始 `125 / 624` 比例由高度计算。
- 已处理：热点文字选择器从 `.map-main__hotspot span` 收窄为 `.map-main__hotspot > span`，避免内部单字 `span` 继承外层热点尺寸。
- 已处理：选中 / 悬停态的滤镜从按钮本体移到内部竖牌素材，并显式清除按钮 focus / active 的 outline 与 box-shadow，避免选中后继续出现外层方形框。
- 已同步：`CHANGELOG.md` v0.5.5 第 3 条；`docs/game-system-breakdown.md` 大地图热点视觉规则；`codex-workdocs/2026-06-29-v055-map-hotspot-ratio-fix.md`。
- 验证：`npx tsc --noEmit` 通过；地图相关 vitest 通过；Playwright DOM 检查确认竖牌宽高比为 `0.2`，按钮背景为 `none`。
